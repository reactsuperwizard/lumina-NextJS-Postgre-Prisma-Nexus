...
;(function rd_Approximate(thisObj) {
  ...
  rd_ApproximateData.strTypeOpts = {
    en: '["Still", "Movie", "File", "Placeholder", "Sequence"]',
  }
  ...
  rd_ApproximateData.strUseSourceNameForProxy = {
    en: 'Use source name for proxy',
  }
  ...
  rd_ApproximateData.strSelectProxyFolder = {
    en: 'Select the folder for storing rendered proxies.',
  }
  rd_ApproximateData.strHelp = { en: '?' }
  rd_ApproximateData.strNoItemToProxy = {
    en:
      'Please select at least one footage or composition item, then try again.',
  }
  rd_ApproximateData.strNoProxiedItemToUnset = {
    en: 'Please select at least one proxied item, then try again.',
  }
  rd_ApproximateData.strErrGetTpls = {
    en:
      "Couldn't retrieve the list of render settings and output module templates.",
  }
  rd_ApproximateData.strErrRenameProxy = {
    en: "Could not rename proxy file to '%1'.",
  }
  rd_ApproximateData.strMinAE100 = {
    en: 'This script requires Adobe After Effects CS5 or later.',
  }
  rd_ApproximateData.strHelpText = {
    en:
      'Copyright (c) 2007-2013 redefinery (Jeffrey R. Almasol).\n' +
      'All rights reserved.\n' +
      '\n' +
  ...
  function rd_Approximate_buildUI(thisObj) {
    var pal =
      thisObj instanceof Panel
        ? thisObj
        : new Window('palette', rd_ApproximateData.scriptName, undefined, {
            resizeable: true,
          })
    ...
    if (pal !== null) {
      var res =
        ...
        rd_ApproximateData.scriptName +
        "', alignment:['fill','center'] }, \
                    help: Button { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strHelp) +
        "', maximumSize:[30,20], alignment:['right','center'] }, \
                }, \
                r1: Group { \
                    alignment:['fill','top'], \
                    proxyType: StaticText { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strType) +
        "' }, \
                    proxyTypeList: DropDownList { properties:{items:" +
        rd_Approximate_localize(rd_ApproximateData.strTypeOpts) +
        "}, alignment:['fill','top'], preferredSize:[-1,20] }, \
                }, \
                r4: Group { \
                    alignment:['fill','top'], \
                    r4left: Group { \
                        ...
                        r4top: Group { \
                            alignment:['fill','top'], \
                            rsTpl: StaticText { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strRSTemplate) +
        "' }, \
                            rsTplList: DropDownList { alignment:['fill','top'], alignment:['fill','top'], preferredSize:[-1,20] }, \
                        }, \
                        r4btm: Group { \
                            alignment:['fill','top'], \
                            omTpl: StaticText { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strOMTemplate) +
        "' }, \
                            omTplList: DropDownList { alignment:['fill','top'], alignment:['fill','top'], preferredSize:[-1,20] }, \
                        }, \
                    }, \
                    refresh: Button { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strRefresh) +
        "', alignment:['right','center'], preferredSize:[-1,20] }, \
                }, \
                r5: Group { \
                    alignment:['fill','top'], \
                    outFolder: StaticText { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strOutFolder) +
        "' }, \
                    outFolderName: EditText { text:'', characters:20, alignment:['fill','top'], preferredSize:[-1,20] }, \
                    outFolderBrowse: Button { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strOutFolderBrowse) +
        "', alignment:['right','top'], preferredSize:[-1,20] }, \
                }, \
                r6: Group { \
                    alignment:['fill','top'], \
                    outName: StaticText { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strOutName) +
        "' }, \
                ...
                r7: Group { \
                    ...
                    proxyNameSameAsSource: Checkbox { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strUseSourceNameForProxy) +
        "' }, \
                }, \
                cmds: Group { \
                    alignment:['right','top'], \
                    unsetProxyBtn: Button { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strUnsetProxy) +
        "', preferredSize:[-1,20] }, \
                    setProxyBtn: Button { text:'" +
        rd_Approximate_localize(rd_ApproximateData.strSetProxy) +
        "', preferredSize:[-1,20] }, \
                }, \
            }"
      pal.grp = pal.add(res)

      pal.grp.r1.proxyType.preferredSize.width = pal.grp.r4.r4left.r4btm.omTpl.preferredSize.width = pal.grp.r5.outFolder.preferredSize.width = pal.grp.r6.outName.preferredSize.width = pal.grp.r7.filler.preferredSize.width =
      ...
      pal.grp.r1.proxyTypeList.onChange = function () {
        ...
        if (proxyType === 0 || proxyType === 4) {
          ...
          this.parent.parent.r6.outNameTpl.text =
        ...
        this.parent.parent.cmds.setProxyBtn.text =
          proxyType === 2
            ? rd_Approximate_localize(rd_ApproximateData.strSetProxyMore)
            : rd_Approximate_localize(rd_ApproximateData.strSetProxy)
      }
      ...
      pal.grp.r5.outFolderBrowse.onClick = function () {
        ...
        if (folder !== null) this.parent.outFolderName.text = folder.fsName
      }
      ...
      pal.grp.header.help.onClick = function () {
        alert(
          rd_ApproximateData.scriptTitle +
            '\n' +
            rd_Approximate_localize(rd_ApproximateData.strHelpText),
          rd_ApproximateData.scriptName,
        )
      }
  ...
  function rd_Approximate_doRefreshTemplates(pal) {
    ...
    for (var i = 0; i < rqi.templates.length; i++)
      if (rqi.templates[i].indexOf('_HIDDEN') !== 0)
    ...
    if (rqi.templates.length > 0)
      // Select the first template in the list, if there is at least one
      pal.grp.r4.r4left.r4top.rsTplList.selection = 0
    if (om.templates.length > 0) pal.grp.r4.r4left.r4btm.omTplList.selection = 0
  ...
  function rd_Approximate_doUnsetProxy() {
    ...
    if (selItems.length === 0) {
      alert(
        rd_Approximate_localize(rd_ApproximateData.strNoProxiedItemToUnset),
        rd_ApproximateData.scriptName,
      )
      return
    }
  ...
  function rd_Approximate_doApproximate() {
    ...
    if (selItems.length === 0) {
      alert(
        rd_Approximate_localize(rd_ApproximateData.strNoItemToProxy),
        rd_ApproximateData.scriptName,
      )
      return
    }
    ...
    if (typeList === 0 || typeList === 1 || typeList === 4) {
      // Still, Movie, Sequence
      ...
      if (outFolder === null || !outFolder.exists) {
        alert(
          rd_Approximate_localize(rd_ApproximateData.strSelectProxyFolder),
          rd_ApproximateData.scriptName,
        )
        return
      }
      ...
      var proxyFile = File.openDialog(
        rd_Approximate_localize(rd_ApproximateData.strFileTitle),
      )
      if (proxyFile === null || !proxyFile.exists) return
    } else if (typeList === 3) {
      // Placeholder
    }
    ...
    if (typeList === 0 || typeList === 1 || typeList === 4) {
      ...
      try {
        ...
        var tempFolder = app.project.items.addFolder(
          rd_ApproximateData.scriptName + ' comps',
        )
        ...
        for (var i = 0; i < selItems.length; i++) {
          ...
          if (typeList === 0) {
            ...
            rqi.timeSpanDuration =
              tempItem.frameRate !== 0 ? tempItem.frameDuration : 1 / 30
          }
        ...
        for (var i = 0; i < rqiList.length; i++) {
          renderedFile = new File(
            rqiList[i].rqItem
              .outputModule(1)
              .file.fsName.replace('[#####]', '00000'),
          )

          if (typeList === 1 && renameProxy && selItems[i].file !== null) {
            // Rename proxy movie based on source footage, if requested
            if (!renderedFile.rename(selItems[i].file.name)) {
              var tempStr = rd_localize(
                rd_ApproximateData.strErrRenameProxy,
              ).replace('%1', selItems[i].file.name)
          ...
          if (renderedFile !== null && renderedFile.exists) {
            if (typeList === 4) {
              selItems[rqiList[i].itemNum].setProxyWithSequence(
                renderedFile,
                false,
              )
            } else {
      ...
      for (var i = 0; i < selItems.length; i++) {
        ...
        if (tempDur === 0) tempDur = 30

        tempFPS = tempItem.frameRate
        if (tempFPS === 0) tempFPS = 30

        tempItem.setProxyWithPlaceholder(
          tempItem.name,
          tempItem.width,
          tempItem.height,
          tempFPS,
  ...
  if (parseFloat(app.version) < 10.0)
    alert(
      rd_Approximate_localize(rd_ApproximateData.strMinAE100),
      rd_ApproximateData.scriptName,
    )
  else {
    ...
    if (rdapPal !== null) {
      ...
      if (
        app.settings.haveSetting('redefinery', 'rd_Approximate_proxyTypeList')
      ) {
        rdapPal.grp.r1.proxyTypeList.selection = parseInt(
          app.settings.getSetting('redefinery', 'rd_Approximate_proxyTypeList'),
        )
      }
      if (app.settings.haveSetting('redefinery', 'rd_Approximate_rsTplList')) {
        rdapPal.grp.r4.r4left.r4top.rsTplList.selection = parseInt(
          app.settings.getSetting('redefinery', 'rd_Approximate_rsTplList'),
        )
      }
      if (app.settings.haveSetting('redefinery', 'rd_Approximate_omTplList')) {
        rdapPal.grp.r4.r4left.r4btm.omTplList.selection = parseInt(
          app.settings.getSetting('redefinery', 'rd_Approximate_omTplList'),
        )
      }
      if (
        app.settings.haveSetting('redefinery', 'rd_Approximate_outFolderName')
      ) {
        rdapPal.grp.r5.outFolderName.text = app.settings
          .getSetting('redefinery', 'rd_Approximate_outFolderName')
          .toString()
      }
      if (app.settings.haveSetting('redefinery', 'rd_Approximate_outNameTpl')) {
        rdapPal.grp.r6.outNameTpl.text = app.settings
          .getSetting('redefinery', 'rd_Approximate_outNameTpl')
          .toString()
      }
      if (
        app.settings.haveSetting(
          'redefinery',
          'rd_Approximate_proxyNameSameAsSource',
        )
      ) {
        rdapPal.grp.r7.proxyNameSameAsSource.value = eval(
      ...
      rdapPal.onClose = function () {
        app.settings.saveSetting(
          'redefinery',
          'rd_Approximate_proxyTypeList',
          rdapPal.grp.r1.proxyTypeList.selection.index.toString(),
        )
        app.settings.saveSetting(
          'redefinery',
          'rd_Approximate_rsTplList',
          rdapPal.grp.r4.r4left.r4top.rsTplList.selection.index.toString(),
        )
        app.settings.saveSetting(
          'redefinery',
          'rd_Approximate_omTplList',
          rdapPal.grp.r4.r4left.r4btm.omTplList.selection.index.toString(),
        )
        app.settings.saveSetting(
          'redefinery',
          'rd_Approximate_outFolderName',
          rdapPal.grp.r5.outFolderName.text,
        )
        app.settings.saveSetting(
          'redefinery',
          'rd_Approximate_outNameTpl',
          rdapPal.grp.r6.outNameTpl.text,
        )
        app.settings.saveSetting(
          'redefinery',
          'rd_Approximate_proxyNameSameAsSource',
          rdapPal.grp.r7.proxyNameSameAsSource.value.toString(),
        )
      }
      ...
      if (rdapPal instanceof Window) {
        ...
        rdapPal.center()
        rdapPal.show()
      } else rdapPal.layout.layout(true)
    }
  }
})(this)
